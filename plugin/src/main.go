// Package main implements the cpa-pool-supervise CLIProxyAPI plugin.
//
...
// credentials. Each subscription exposes two CPA credentials (one per
// protocol) that share the same 5-hour/weekly/monthly quota. The plugin
// groups them into logical accounts, synchronizes health across both
// credentials, blocks accounts on quota exhaustion (passively from 429
// responses and optionally proactively from dashboard quota polling), and
// exposes a management API plus a status page.
package main
