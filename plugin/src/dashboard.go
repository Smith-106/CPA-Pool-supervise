...
func pollLoop(stop <-chan struct{}, kick <-chan struct{}) {
	...
	for {
		...
		var targets []*account
		var ollamaTargets []*account
		for _, acct := range p.accounts {
			if acct.Disabled {
				continue
			}
			if acct.Kind == "ollama" {
				// Ollama usage is reachable with the API key alone; always
				// poll it, no dashboard credentials needed.
				ollamaTargets = append(ollamaTargets, acct)
				continue
			}
			workspaceID, cookie, cookieFile := p.dashboardCredentials(acct)
			if workspaceID != "" && (cookie != "" || cookieFile != "") {
				targets = append(targets, acct)
			}
		}
		p.mu.Unlock()

		if time.Since(lastDashboard) >= interval && (len(targets) > 0 || len(ollamaTargets) > 0) {
			lastDashboard = time.Now()
			for _, acct := range targets {
				refreshAccount(p, acct)
			}
			for _, acct := range ollamaTargets {
				refreshOllamaAccount(p, acct)
			}
		}
