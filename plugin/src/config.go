...
// account is one logical upstream credential pool member. For OpenCode Go it
// is a pair of CPA credentials (openai-compatibility + claude-api-key) that
// share the same quota windows; for Ollama it is a single API key whose
// monthly quota is polled from https://ollama.com/api/usage.
type account struct {
	Name        string
	KeySuffix   string
	Kind        string // "opencode" or "ollama"
	OpenAIID    string
	ClaudeID    string
	WorkspaceID string
	CookieFile  string
	...
	// apiKey is kept in memory only for override matching and Ollama usage
	// polling; it must never be serialized into status output or logs.
	apiKey string
}
...
func discoverAccounts(cfg settings) ([]*account, string, error) {
	...
	for i := range cpa.OpenAICompatibility {
		...
		for j := range compat.APIKeyEntries {
			...
			isOllama := strings.HasPrefix(providerName, ollamaProviderPrefix)
			if providerName != opencodeCompatName && !isOllama {
				continue
			}
			acct := ensure(key)
			if isOllama {
				// Ollama keys are single-credential accounts; no claude pairing.
				acct.OpenAIID = id
				acct.Kind = "ollama"
				if acct.Name == "" {
					acct.Name = providerName + "-" + acct.KeySuffix
				}
			} else {
				acct.OpenAIID = id
			}
