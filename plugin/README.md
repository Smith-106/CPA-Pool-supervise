# CPA Pool Supervise for CLIProxyAPI

`cpa-pool-supervise` is a quota-aware [CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI)
...
plugins:
  ...
  dir: plugins
  store-sources:
    - https://raw.githubusercontent.com/Smith-106/CPA-Pool-supervise/main/registry.json
```

Reload or restart CPA, open **CPA Manager Plus → Plugin Store**, refresh the
store, and install **CPA Pool Supervise**. The installer verifies the release
...
Copy `dist/cpa-pool-supervise-v0.3.0.so` into CPA's
...
are stored under `<auth-dir>/cpa-pool-supervise`.
...
```text
/v0/resource/plugins/cpa-pool-supervise/status
```
...
- it is saved only in `<auth-dir>/cpa-pool-supervise/settings.json` with mode `0600`;
...
| Route | Purpose |
| --- | --- |
| `GET /v0/management/plugins/cpa-pool-supervise/status` | Return pool state |
| `POST /v0/management/plugins/cpa-pool-supervise/refresh` | Refresh dashboard usage |
| `POST /v0/management/plugins/cpa-pool-supervise/unblock` | Clear one account's plugin-level blocks |
| `POST /v0/management/plugins/cpa-pool-supervise/account-config` | Save or clear dashboard settings |
...
`cpa-pool-supervise_<version>_linux_amd64.zip` and `checksums.txt` in `dist/`
